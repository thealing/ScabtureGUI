  ...
  if (file == NULL) {
    ...
    switch (logging_mode) {
      case LOGGING_SINGLE_FILE: {
        _swprintf(log_name, L"CaptureBit Log.txt");
        break;
      }
      case LOGGING_DATE_FILE: {
        SYSTEMTIME time;
        GetSystemTime(&time);
        _swprintf(log_name, L"CaptureBit Log [%04hi.%02hi.%02hi.] [%02hi.%02hi.%02hi.%03hi].txt", time.wYear, time.wMonth, time.wDay, time.wHour, time.wMinute, time.wSecond, time.wMilliseconds);
        break;
      }
    }
