...
const wchar_t* FileUtil::generateRecordingSavePath()
{
	...
	UniquePointer<const wchar_t> path = StringUtil::formatString(L"%ls\\CaptureBit", videosFolder);
...
const wchar_t* FileUtil::generateSnapshotSavePath()
{
	...
	UniquePointer<const wchar_t> path = StringUtil::formatString(L"%ls\\CaptureBit", picturesFolder);
